...
export default async function PostPage({ params }: Props) {
  const { slug } = await params
  const post = await getBlogPost(slug)
  const item = post ? { title: plainText(post.title.rendered), excerpt: plainText(post.excerpt.rendered), date: post.date, content: post.content.rendered } : fallback[slug]
  if (!item) notFound()

  return <div className="min-h-screen bg-[#f6f7f2] text-[#10130f]"><Header /><main id="main-content" className="pt-20"><article><header className="border-b border-black/10 px-5 py-16 md:px-10 md:py-24"><div className="mx-auto max-w-4xl"><Link href="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-black/50 hover:text-black"><ArrowLeft className="h-4 w-4" /> All insights</Link><p className="eyebrow mt-16">Insight · {new Date(item.date).toLocaleDateString("en-CA", { month: "long", day: "numeric", year: "numeric" })}</p><h1 className="mt-5 text-5xl font-semibold leading-[.98] tracking-[-.055em] md:text-7xl">{item.title}</h1><p className="mt-8 max-w-3xl text-xl leading-relaxed text-black/55 md:text-2xl">{item.excerpt}</p></div></header><div className="px-5 py-16 md:px-10 md:py-24"><div className="article-content mx-auto max-w-3xl" dangerouslySetInnerHTML={{ __html: item.content }} /></div></article></main><Footer /></div>
}
